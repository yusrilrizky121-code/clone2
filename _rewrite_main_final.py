...
void main() async {
  ...
  // Request notifikasi permission Android 13+
...
class _AuspotyWebViewState extends State<AuspotyWebView>
  ...
  void initState() {
    ...
    // Callback dari tombol notifikasi media
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused) {
      // App ke background — audio tetap jalan via foreground service
      _progressTimer?.cancel();
      WakelockPlus.enable();
    }
    if (state == AppLifecycleState.resumed) {
      // Kembali ke foreground — sync UI
      _startProgressTimer();
      _syncPlaybackState();
    }
  }
  ...
  void _startProgressTimer() {
    ...
    _progressTimer = Timer.periodic(const Duration(seconds: 1), (_) async {
      if (!audioHandler.isPlaying) return;
      final pos = audioHandler.position.inMilliseconds;
      ...
      if (dur > 0 && _wvc != null) {
        ...
        await _wvc!.evaluateJavascript(source: """
          (function(){
            ...
            var fmt = function(ms){ var s=Math.floor(ms/1000),m=Math.floor(s/60),sec=s%60; return m+':'+(sec<10?'0':'')+sec; };
            if(bar){ bar.value=$pct; bar.style.background='linear-gradient(to right,white ${pct}%,rgba(255,255,255,0.2) ${pct}%)'; }
            if(ct) ct.innerText=fmt($pos);
            if(tt) tt.innerText=fmt($dur);
          })();
        """);
      }
    });
          ...
          child: Stack(children: [
            InAppWebView(
              ...
              onWebViewCreated: (c) {
                ...
                // ── playStream: JS kirim URL → Flutter putar + notifikasi ──
                c.addJavaScriptHandler(
                  ...
                  callback: (args) async {
                    ...
                    await audioHandler.playUrl(url, item);
                    _startProgressTimer();
                ...
                c.addJavaScriptHandler(
                  ...
                  callback: (args) async {
                    final pct = args.isNotEmpty
                        ? double.tryParse(args[0].toString()) ?? 0.0
                        : 0.0;
                    final dur = audioHandler.duration.inMilliseconds;
                    if (dur > 0) {
                      await audioHandler.seek(
                          Duration(milliseconds: (pct / 100 * dur).round()));
                    }
                  },
                );

                c.addJavaScriptHandler(
                    handlerName: 'isAndroid', callback: (args) => true);

                c.addJavaScriptHandler(
                  handlerName: 'onMusicPlaying',
                  callback: (args) async => WakelockPlus.enable(),
                );
                c.addJavaScriptHandler(
                  handlerName: 'onMusicPaused',
                  callback: (args) async => _progressTimer?.cancel(),
                );
                c.addJavaScriptHandler(
                  handlerName: 'onMusicResumed',
                  callback: (args) async => _startProgressTimer(),
                );
              ...
              shouldOverrideUrlLoading: (c, nav) async {
                ...
                const ok = [
                  'vercel.app', 'youtube.com', 'ytimg.com', 'googleapis.com',
                  'gstatic.com', 'firebaseapp.com', 'firebase.google.com',
                  'accounts.google.com', 'google.com', 'googleusercontent.com'
                ];
                for (final d in ok) {
                  if (url.contains(d)) return NavigationActionPolicy.ALLOW;
                }
...
open('auspoty-flutter/lib/main.dart', 'w', encoding='utf-8').write(main_dart)
print("main.dart written!")
print(f"Lines: {len(main_dart.splitlines())}")
