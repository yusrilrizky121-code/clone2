...
function playMusic(videoId, encodedData) {
    ...
    // Putar via YouTube IFrame (web + APK)
    ...
    if (window.AndroidBridge && typeof window.AndroidBridge.onMusicPlay === 'function') {
        window.AndroidBridge.onMusicPlay(
...
function togglePlay() {
    ...
    if (isPlaying) {
        ytPlayer.pauseVideo();
        if (window.AndroidBridge && typeof window.AndroidBridge.onMusicPause === 'function') {
            window.AndroidBridge.onMusicPause();
        }
    } else {
        ytPlayer.playVideo();
    }
...
function downloadMusic() {
    ...
    if (window.AndroidBridge && typeof window.AndroidBridge.downloadFile === 'function') {
        showToast('Mengunduh ' + currentTrack.title + '...');
        window.AndroidBridge.downloadFile(currentTrack.videoId, currentTrack.title || 'musik');
    } else {
        // fallback web
